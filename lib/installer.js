...
function computeVersion(version, includePreReleases, repoToken) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        process.stdout.write("allVersions:" + allVersions + os.EOL);
        const possibleVersions = allVersions.filter(v => v.startsWith(version));
        process.stdout.write("possibleVersions: " + possibleVersions + os.EOL);
        const versionMap = new Map();
        possibleVersions.forEach(v => versionMap.set(normalizeVersion(v), v));
        process.stdout.write("versionMap: " + versionMap + os.EOL);
        const versions = Array.from(versionMap.keys())
            .sort(semver.rcompare)
            .map(v => versionMap.get(v));
        process.stdout.write("versions: " + versions + os.EOL);
